...
def M_price_el_impute(ds, X, xis, theta, avg_price_el, all_markets=True):
...
def M_data_cons(ds, X, xis, theta, avg_price_el, all_markets=True):
...
def M_data_cons_q(ds, X, xis, theta, avg_price_el, all_markets=True):
    # determine E[x*] and shares
    qidx = ds.chars.index(ds.qname)
    Q = X[:,:,qidx]
    dlimidx = ds.chars.index(ds.dlimname)
...
def M_pop_dens(ds, X, xis, theta, avg_price_el, all_markets=True):
...
def M_dlim(ds, X, xis, theta, avg_price_el, all_markets=True):
    xis_O = xis[:,ds.Oproducts]
    dlimidx = ds.chars.index(ds.dlimname)
...
def M_vlim(ds, X, xis, theta, avg_price_el, all_markets=True):
    xis_O = xis[:,ds.Oproducts]
...
def M_O(ds, X, xis, theta, avg_price_el, all_markets=True):
    xis_O = xis[:,ds.Oproducts]
...
def M_yc5(ds, X, xis, theta, avg_price_el, all_markets=True):
    xis_O = xis[:,ds.Oproducts]
...
def M_data_cons_yc5(ds, X, xis, theta, avg_price_el, all_markets=True):
    yc2idx = ds.dim3.index(ds.demolist[1])
    yc5idx = ds.dim3.index(ds.demolist[4])
    yc8idx = ds.dim3.index(ds.demolist[7])
    yc2_jm = X[:,ds.Oproducts,yc2idx]
...
def M_avg_q_other(ds, X, xis, theta, avg_price_el, all_markets=True):
    xis_O = xis[:,ds.Oproducts]
    qidx = ds.chars.index(ds.qname)
