...
def M_data_cons_dlim(ds, X, xis, theta, avg_price_el, div_ratio, all_markets=True):
    # determine E[x*] and shares
    qidx = ds.chars.index(ds.qname)
    Q = X[:,:,qidx]
    dlimidx = ds.chars.index(ds.dlimname)
    ...
    mom = np.mean((predicted_dbar[:,ds.Oproducts] - (X[:,ds.Oproducts,dbaridx] / de.conv_factor)) * (dlim_jm / de.conv_factor), axis=1, keepdims=True)
    if all_markets:
        mom = mom[ds.markets_moms,:]
    return mom

# Difference in predicted data consumed and observed data consumed shouldn't vary by quality
def M_data_cons_q(ds, X, xis, theta, avg_price_el, div_ratio, all_markets=True):
    # determine E[x*] and shares
    qidx = ds.chars.index(ds.qname)
    Q = X[:,:,qidx]
    dlimidx = ds.chars.index(ds.dlimname)
...
# Demand shock should be uncorrelated with median income and data limit (helps to identify the price heterogeneity term)
def M_yc5_dlim(ds, X, xis, theta, avg_price_el, div_ratio, all_markets=True):
    xis_O = xis[:,ds.Oproducts]
    yc2idx = ds.dim3.index(ds.demolist[1])
    yc5idx = ds.dim3.index(ds.demolist[4])
    yc8idx = ds.dim3.index(ds.demolist[7])
    ...
    dlimidx = ds.chars.index(ds.dlimname)
    dlim_jm = X[:,ds.Oproducts,dlimidx]
    mom5 = np.mean(xis_O * yc5_jm / coef.income_conv_factor * dlim_jm / de.conv_factor, axis=1, keepdims=True)
    mom = mom5
    if all_markets:
        mom = mom[ds.markets_moms,:]
    return mom
...
# Difference between predicted and observed data consumption should be uncorrelated with median income and data limit (helps to identify the data heterogeneity term)
def M_data_cons_yc5_dlim(ds, X, xis, theta, avg_price_el, div_ratio, all_markets=True):
    yc2idx = ds.dim3.index(ds.demolist[1])
    yc5idx = ds.dim3.index(ds.demolist[4])
    yc8idx = ds.dim3.index(ds.demolist[7])
    yc2_jm = X[:,ds.Oproducts,yc2idx]
    ...
    dbaridx = ds.dim3.index(ds.dbarname)
    dlim_jm = X[:,ds.Oproducts,dlimidx]
    mom5 = np.mean(yc5_jm / coef.income_conv_factor * (predicted_dbar[:,ds.Oproducts] - (X[:,ds.Oproducts,dbaridx] / de.conv_factor) * dlim_jm / de.conv_factor), axis=1, keepdims=True)
    mom = mom5
    if all_markets:
        mom = mom[ds.markets_moms,:]
    return mom
...
def M_avg_q_other(ds, X, xis, theta, avg_price_el, div_ratio, all_markets=True):
    xis_O = xis[:,ds.Oproducts]
    qidx = ds.chars.index(ds.qname)
...
# moments = (M_price_el_impute, M_div_ratio, M_data_cons, M_data_cons_dlim, M_dlim, M_vlim, M_O, M_yc5, M_data_cons_yc5, M_pop_dens)
# K = 10
