...
def G_n(ds, theta, X, avg_price_el, sigma, N, K, eps):
    ...
    for n in range(N):
        g_0 = gmm.g(ds, theta, X[n,:,:][np.newaxis,:,:], avg_price_el, sigma, nonOxis={'firms': np.array([2,3,4,5]), 'xis': nonOxis})
        for i in range(len(theta)):
            epsilon = np.zeros(len(theta))
            epsilon[i] = eps
            grad = (gmm.g(ds, theta + epsilon, X[n,:,:][np.newaxis,:,:], avg_price_el, sigma, nonOxis={'firms': np.array([2,3,4,5]), 'xis': nonOxis}) - g_0) / eps
            g_jac[n, :, i] = grad
