...
def G_n(ds, theta, X, avg_price_el, div_ratio, N, K, eps):
    g_jac = np.zeros((N, K, len(theta)))
    xis = blp.xi(ds, theta, X, None)
    nonOxis = xis[0,np.unique(ds.firms, return_index=True)[1][1:]]
    for n in range(N):
        g_0 = gmm.g(ds, theta, X[ds.markets_moms,:,:][n,:,:][np.newaxis,:,:], avg_price_el, div_ratio, nonOxis={'firms': np.array([2,3,4,5]), 'xis': nonOxis}, all_markets=False)
        for i in range(len(theta)):
            epsilon = np.zeros(len(theta))
            epsilon[i] = eps
            grad = (gmm.g(ds, theta + epsilon, X[ds.markets_moms,:,:][n,:,:][np.newaxis,:,:], avg_price_el, div_ratio, nonOxis={'firms': np.array([2,3,4,5]), 'xis': nonOxis}, all_markets=False) - g_0) / eps
            g_jac[n, :, i] = grad
