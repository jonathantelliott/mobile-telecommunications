...
def W(ds, thetahat, K, avg_price_el, sigma):
    ...
    for n in range(N):
        demeanedmoment = gmm.g(ds, thetahat, ds.data[ds.markets_moms,:,:][n,:,:][np.newaxis,:,:], avg_price_el, sigma, nonOxis={'firms': np.array([2,3,4]), 'xis': nonOxis}, all_markets=False) - ghat
